...
export async function GET(request: Request) {
  ...
  const upstreamUrl = `${getApiBase()}/applications/${qp ? `?${qp}` : ''}`
...
export async function POST(request: Request) {
  ...
  try {
    const resp = await fetch(`${getApiBase()}/applications/`, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
    ...
    try { data = text ? JSON.parse(text) : null } catch {}
    if (!resp.ok) {
      return NextResponse.json({ message: data?.detail || data?.message || 'Failed to create application' }, { status: resp.status || 400 })
    }
