...
export async function GET() {
  ...
  try {
    const resp = await fetch(`${apiBase()}/applications/me`, {
      method: 'GET',
    ...
    if (!resp.ok) {
      return NextResponse.json({ message: data?.detail || data?.message || 'Failed to fetch my applications' }, { status: resp.status || 500 })
    }
